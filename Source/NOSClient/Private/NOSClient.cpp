...
void FNOSClient::Disconnected_GrpcThread()
{
	...
	// The peer can no longer advance shared GPU timelines. Release external waits
	// immediately on this callback thread so a blocked render/RHI thread cannot
	// prevent the game-thread connection-close task from running.
