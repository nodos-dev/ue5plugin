...
struct ExecuteFrameNumberQueue : public TQueue<ExecuteInfo>
{
	...
	bool WaitForFrame()
	{
		...
		constexpr auto ReconnectTickInterval = std::chrono::milliseconds(100);
		const bool bWoken = FrameAvailable.wait_for(lock, ReconnectTickInterval,
			[this]() { return !bSynchronized || !IsEmpty(); });
