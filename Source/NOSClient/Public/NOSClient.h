...
struct ExecuteFrameNumberQueue : public TQueue<ExecuteInfo>
{
	...
	void EnqueueExecuteStart(nos::app::AppExecuteStart const* appExecuteStart)
	{
		...
		const bool bReset = appExecuteStart->reset();
		{
			...
			if (bReset)
			{
				// This marker means a Nodos path stopped; it is not itself a synchronization
				// epoch reset. Nodos still accounts for every AppExecuteStart already sent,
				// so keep the queued requests and return their completions. A real IDLE
				// transition clears the queue in ResetForNewSyncEpoch.
				// Refilling a deep ring can take several Unreal frames before Nodos can
				// resume issuing requests, so give that recovery a bounded number of
				// ordinary ticks. This never paces normal synchronized frames.
	...
	bool WaitForFrame()
	{
		...
		// The game thread services reconnection and queued state changes, so an
		// unbounded wait here would block the very work that recovers the connection.
		// State changes and connection close wake it immediately; this timeout only
		// catches a callback that never arrives. It is a deadlock watchdog, not frame
		// pacing.
		const int32 DeadlockWatchdogTimeoutMs = GetNodosDeadlockWatchdogTimeoutMs();
		const bool bWoken = FrameAvailable.wait_for(lock, std::chrono::milliseconds(DeadlockWatchdogTimeoutMs),
