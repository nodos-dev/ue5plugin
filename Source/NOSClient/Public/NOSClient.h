	...
	void EnqueueExecuteStart(nos::app::AppExecuteStart const* appExecuteStart)
	{
		...
		const bool bReset = appExecuteStart->reset();
		{
			...
			if (bReset)
			{
				...
				// and return ExecutionCompleted. The reset still has to wake the game thread:
				// path restart work is serviced there, so ignoring this marker deadlocks Nodos
				// until the emergency watchdog releases Unreal.
